...
Магазин PlayStation Store кодов пополнения — клон backdoor.store.
...
```bash
git clone https://github.com/NickLuck777/backdoor-store.git
cd backdoor-store
cp .env.example .env
```
...
- Admin: http://localhost:3000/admin (login: admin@backdoor.store / Admin123!)
